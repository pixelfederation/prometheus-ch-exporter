...
- A **`ClickHouseQuery`** references a connection by name and defines an SQL query
  ...
  failover); **system** queries fan out to every node (adding a `node` label).
...
**System query** — fans out to every node, adds a `node` label:
...
spec:
  ...
  metric:
    name: clickhouse_node_uptime_seconds
    help: "ClickHouse uptime in seconds, per node"
```

The example above exposes:

```
app_errors{env="production",app="checkout",query_key="monitoring/errors-by-app"} 12
clickhouse_node_uptime_seconds{node="10.0.0.1",query_key="monitoring/uptime-node"} 512345
```
...
Besides your user metrics, the operator always exposes:

| Metric | Labels | Meaning |
|---|---|---|
| `ch_query_up` | `query_key` | 1 if the last run succeeded, else 0 |
| `ch_query_last_success_timestamp_seconds` | `query_key` | Unix time of last success |
| `ch_query_duration_seconds` | `query_key` | Last run duration |
| `ch_query_inflight` | `query_key` | Executions currently running |
| `ch_query_skipped_total` | `query_key` | Ticks skipped because `maxConcurrent` was reached |
...
- **Query:** `phase` (`Pending` / `Healthy` / `Degraded` / `Failing` /
  `Expired`), `lastSuccess`, `lastError`, `failedNodes`, and conditions
  (`QuerySucceeded`, `AllNodesResponding`, `KeepingUp`, `Ready`). Phase
  transitions are also emitted as Kubernetes Events.
